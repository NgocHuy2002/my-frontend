      ...
      const queryParams = Object.entries(search)
        .map(([key, value]) => {
          if (typeof value === "object") {
            // If the value is an object, stringify it without quotes
            return `${encodeURIComponent(key)}=${encodeURIComponent(
              JSON.stringify(value)
            )}`;
          } else {
            // If the value is not an object, stringify it as is
  ...
  const captureModalContent = async () => {
    ...
    html2canvas(modalContentRef.current).then((canvas) => {
      ...
      canvas.toBlob((blob) => {
        ...
        formData.append("image", blob, "modal_content.png");
    ...
    <>
      ...
      >
        <div ref={modalContentRef} style={{ padding: 15 }}>
          <Form layout="vertical" form={form} onFinish={handleSend} disabled>
            <Form.Item
              label="Tên sản phẩm"
              name="name"
              rules={[
