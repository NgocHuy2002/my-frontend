import { Card, Table, Tag, Space, message, Image, Col, Row, Button, Modal, Form, Select, DatePicker, Input } from "antd";
...
const KiemDuyet = () => {
  ...
  const handleModel = (value) => {
    ...
    form.setFieldsValue({
      sendTo: value.sendTo,
      sendBy: value.sendBy,
      filename: decodeURIComponent(escape(value.filename)).replace(/\.png$/, ''),
          ...
          {idImage.map((e) => (
            <Col key={e._id} span={6} style={{ borderStyle: 'solid', borderWidth: 1, margin: 10, padding: 5 }}>
              ...
              <div style={{ textAlign: "center" }}>
                ...
                />
                <h5>{decodeURIComponent(escape(e.filename)).replace(/\.png$/, '')}</h5>
                <Button type="primary" onClick={() => handleModel(e)}>Chi tiết</Button>
              </div>
            </Col>
          ))}
        </Row>
